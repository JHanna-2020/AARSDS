...
$session_score = isset($_SESSION['lesson_score']) ? $_SESSION['lesson_score'] : 0;

$stmt = $con->prepare('SELECT grade, lesson_score, lesson_is_submitted FROM accounts WHERE id = ?'); 
$stmt->bind_param('i', $_SESSION['account_id']); 
$stmt->execute(); 
$stmt->bind_result($grade, $current_lesson_score, $lesson_is_submitted); 
$stmt->fetch(); 
$stmt->close();  
...
if ($session_score > 0 && !$lesson_is_submitted) {
    $new_score = $current_lesson_score + $session_score;

    $updateStmt = $con->prepare('UPDATE accounts SET lesson_score = ?, lesson_is_submitted = 1 WHERE id = ?'); 
    $updateStmt->bind_param('ii', $new_score, $_SESSION['account_id']); 
    $updateStmt->execute(); 
    $updateStmt->close();
    
    // Clear session score after successful update
    unset($_SESSION['lesson_score']);
} else {
    // Already submitted or no session score, use current database score
    $new_score = $current_lesson_score;
}
